...
def locIsFree(con):
    ...
    try:
        query="select * from event where building_name=\'"""+building+"""\' and room_number=\'"""+room +"""\' and event_date = \'"""+ date +"""\' and 
( 
...
def groupInvited(con):
    ...
    try: 
        query="select event_name from user_group join event on user_group.group_id = event.group_id where event_name = \'" + event_name + "\' and user_group.name = \'" + group_name +"\';"
        rows = cur.execute(query)
        if(rows > 0):  
            print("Yes\n")
        else :
